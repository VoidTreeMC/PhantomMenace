...
public class PhantomEvent extends BukkitRunnable {
  ...
  public void run() {
    ...
    if (waveIndex < waveList.size()) {
      System.out.println("Attempting to spawn wave...");
      waveList.get(waveIndex++).spawnWave(loc);
      Bukkit.broadcastMessage("Beginning wave " + (waveIndex));
  ...
  public static void manageKill(Phantom phantom, Player player) {
    if (phantom.hasMetadata(EVENT_METADATA_KEY) && player != null) {
      ...
      int numPlayerKilledThisWave = (waveKillMapList.get(getWaveIndex()).get(playerUUID) == null) ? 0 : waveKillMapList.get(getWaveIndex()).get(playerUUID);
      waveKillMapList.get(getWaveIndex()).put(playerUUID, numPlayerKilledThisWave + 1);
    ...
    System.out.println(numKilledThisWave + " / " + totalThisWave);
    ...
    if (numKilledThisWave >= totalThisWave) {
      ...
      Bukkit.broadcastMessage("Wave " + getWaveIndex() + " completed!");
      printTopFive();
    }
  }
  ...
  public static void printTopFive() {
    int num = getWaveIndex();
    Bukkit.broadcastMessage("Top 5 players for Wave " + num);
    Bukkit.broadcastMessage("=======================");
    SortedSet<Map.Entry<UUID, Integer>> sorted = entriesSortedByValues(waveKillMapList.get(num));
    int i = 0;
    for (Map.Entry<UUID, Integer> entry : sorted) {
      ...
      Bukkit.broadcastMessage(playerName + ": " + numKills + " phantoms killed.");
    }
